  ...
  try {
    ...
    if (statusesRequiringGPS.includes(status)) {
      if (!location || !location.latitude || !location.longitude) {
        return res.status(400).json({
          success: false,
          message: 'Position GPS obligatoire pour ce statut (prévention fraude)',
    ...
    if (location && location.latitude && location.longitude && worksite.location?.coordinates) {
      ...
      // Vérifications de cohérence selon le statut
      if (status === 'arrived' && distance > 0.5) {
        return res.status(400).json({
          success: false,
          message: `Vous êtes trop loin du chantier (${distance.toFixed(1)}km). Pour marquer "arrivé", vous devez être à moins de 500m.`,
      ...
      if (status === 'work_started' && distance > 0.2) {
        return res.status(400).json({
          success: false,
          message: `Vous devez être sur le chantier pour démarrer les travaux (distance: ${distance.toFixed(1)}km). Maximum 200m.`,
          distance: distance.toFixed(2),
          maxDistance: 0.2,
        });
      }

      // Vérification mouvement pour "en route" (pas au même endroit qu'avant)
      if (status === 'en_route' && worksite.workerCurrentLocation?.coordinates) {
        ...
        if (moved < 0.1 && timeSinceLastUpdate < 60) {
          console.warn(`⚠️ Possible fraude: worker n'a bougé que de ${(moved*1000).toFixed(0)}m`);
        }
      }
    }
...
exports.updateWorkerLocation = async (req, res) => {
  try {
    ...
    // Mise à jour position uniquement (pas d'historique, juste position actuelle)
    worksite.workerCurrentLocation = {
      type: 'Point',
    ...
    console.error('❌ Erreur updateWorkerLocation:', error);
    res.status(500).json({
      success: false,
